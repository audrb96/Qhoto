...
public interface QuestRepository extends JpaRepository<Quest,Long> {
    ...
    @Query("select new com.qhoto.qhoto_api.dto.response.QuestOptionItemRes(a.id, q.name, q.questType.name) from Quest q inner join fetch ActiveWeekly a on q.id = a.quest.id where a.status = 'A'")
    ...
    @Query("select new com.qhoto.qhoto_api.dto.response.QuestOptionItemRes(a.id, q.name, q.questType.name) from Quest q inner join fetch ActiveMonthly a on q.id = a.quest.id where a.status = 'A'")
    ...
    @Query("select new com.qhoto.qhoto_api.dto.response.QuestOptionItemRes(a.id, q.name, q.questType.name) from Quest q inner join fetch ActiveDaily a on q.id = a.quest.id where a.status = 'A'")
