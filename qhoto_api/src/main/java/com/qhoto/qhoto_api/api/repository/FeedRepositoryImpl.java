...
//                        feed.time,
                        ...
                        user.expGrade,
                        user.totalExp,
                        new CaseBuilder().when(JPAExpressions.select(feedLike).from(feedLike,feed).where(feedLike.feed.id.eq(feed.id),feedLike.user.id.eq(userId)).exists()).then(LikeStatus.LIKE).otherwise(LikeStatus.UNLIKE).as("likeStatus"),
